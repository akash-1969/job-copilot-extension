...
function matchRoleScore(titleLower, targetRoles, userProfile) {
  ...
  for (const role of targetRoles) {
    ...
    if (titleLower === roleLower) {
      ...
      bestRoleMatch = Math.max(bestRoleMatch, 25);
    } else {
      // Token overlap
...
function generateCareerInsights(jobDetails, userProfile, analysis, matchDetails) {
  ...
  const userSkills = (userProfile.skills || '').split(',').map(s => s.trim().toLowerCase()).filter(s => s.length > 0);
  
  const explicitReq = analysis.extractedSkills || [];
  const inferredReq = analysis.inferredSkills || [];
  
  const { explicitMatched, explicitMissing, inferredMatched, inferredMissing, roleScore, locScore, remoteScore } = matchDetails;
  ...
  topMissing.forEach(item => {
    ...
    if (explicitReq.length > 0) {
      const explicitScore = Math.round((expMatchedNew.length / explicitReq.length) * 25);
      const inferredScore = inferredReq.length > 0 ? Math.round((infMatchedNew.length / inferredReq.length) * 15) : 15;
      newSkillScore = explicitScore + inferredScore;
    } else {
      newSkillScore = inferredReq.length > 0 ? Math.round((infMatchedNew.length / inferredReq.length) * 40) : 40;
    }
    
    const newFinalScore = Math.min(100, newSkillScore + roleScore + locScore + remoteScore);
  ...
  const missingKeywords = COMMON_SKILLS
...
function calculateEstimatedMatch(jobCardData, userProfile) {
  ...
  // 1. Inferred Skill Match (Max 40) using the central classification from Job Understanding Layer
  const classify = window.classifyRoleByTitle || (() => null);
  const roleKey = classify(title);
  
  const kb = window.ROLE_KNOWLEDGE_BASE || { software: [] };
  let inferredSkills = [];
  if (roleKey && kb[roleKey]) {
    inferredSkills = kb[roleKey];
  } else {
    inferredSkills = kb['software'] || [];
  }
  ...
  const matchedInferred = inferredSkills.filter(s => userSkills.includes(s));
  skillScore = inferredSkills.length > 0 ? Math.round((matchedInferred.length / inferredSkills.length) * 40) : 40;
  ...
  const rawScore = skillScore + roleScore + locScore + remoteScore;
  
  return {
    score: rawScore,
    breakdown: {
      skills: { score: skillScore, max: 40, pct: Math.round((skillScore / 40) * 100) },
      role: { score: roleScore, max: 30, pct: Math.round((roleScore / 30) * 100) },
...
function calculateFullMatch(jobDetailsData, userProfile) {
  ...
  // Scoring weights: Explicit (25 pts max), Inferred (15 pts max)
  // Fallback: If no explicit skills are present in the job description, Inferred takes the full 40 pts
  if (explicitReq.length > 0) {
    const explicitScore = Math.round((explicitMatched.length / explicitReq.length) * 25);
    const inferredScore = inferredReq.length > 0 ? Math.round((inferredMatched.length / inferredReq.length) * 15) : 15;
    skillScore = explicitScore + inferredScore;
  } else {
    skillScore = inferredReq.length > 0 ? Math.round((inferredMatched.length / inferredReq.length) * 40) : 40;
  }
  ...
  const finalScore = skillScore + roleScore + locScore + remoteScore;
  ...
  if (missingSkillsDemand.length > 0) {
    ...
    topMissing.forEach((item, idx) => {
      ...
      detectedJobs.forEach(job => {
        ...
        const finalScore = Math.min(100, newSkillScore + roleScore + locScore + remoteScore);
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  window.JobCopilotMatchEngine = exports;
}
